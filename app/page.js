...
export default function MarketZoneApp() {
  ...
  const handleVendorRegister = (e) => {
    ...
    if (!storeName.trim() || !ownerName.trim() || !gstNumber.trim()) {
      alert('મહેરબાની કરીને સ્ટોરનું નામ, માલિકનું નામ અને GST નંબર દાખલ કરો.');
      return;
    }
    setIsVendorRegistered(true);
    alert('🏪 વેન્ડર સ્ટોર અને KYC સફળતાપૂર્વક વેરિફાઈડ થઈ ગયા છે!');
  };
  ...
  const addProd = (e) => {
    ...
    setProds([{
      ...
      desc: 'MarketZone પર KYC વેરિફાઈડ વેન્ડર દ્વારા પબ્લિશ કરેલ પ્રોડક્ટ.'
    }, ...prods]);
