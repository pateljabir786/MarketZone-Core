...
export default function MarketZoneApp() {
  ...
  const handleVendorRegister = (e) => {
    ...
    if (!storeName.trim() || !ownerName.trim()) {
      alert('મહેરબાની કરીને સ્ટોરનું નામ અને માલિકનું નામ દાખલ કરો.');
      return;
    }
    setIsVendorRegistered(true);
    alert('🏪 વેન્ડર સ્ટોર સફળતાપૂર્વક રજિસ્ટ્રાઈઝ થઈ ગયો છે!');
  };

  const addProd = (e) => {
    e.preventDefault();
    if (!title.trim() || !price) {
      alert('મહેરબાની કરીને પ્રોડક્ટનું નામ અને કિંમત ભરો.');
      return;
    }
    setProds([{
      id: Date.now(),
      ...
      desc: 'MarketZone પર રજિસ્ટ્રાઈઝ્ડ વેન્ડર દ્વારા પબ્લિશ કરેલ પ્રોડક્ટ.'
    }, ...prods]);
                ...
                <div key={item.id} style={{ backgroundColor: '#0f172a', border: '1px solid #1e293b', padding: '20px', borderRadius: '14px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '15px', flexWrap: 'wrap' }}>
                  <div onClick={() => setSelectedProduct(item)} style={{ cursor: 'pointer', flex: 1 }}>
                    <div style={{ display: 'flex', gap: '8px', alignItems: 'center', marginBottom: '10px', flexWrap: 'wrap' }}>
                      ...
                      <span style={{ fontSize: '10px', backgroundColor: '#022c22', border: '1px solid #059669', padding: '4px 8px', borderRadius: '6px', fontWeight: 'bold', color: '#34d399' }}>🛡️ Verified Legal</span>
                    </div>
                  ...
                  <button onClick={() => setCart([...cart, item])} style={{ backgroundColor: '#2563eb', color: '#fff', border: 'none', padding: '10px 18px', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer', fontSize: '13px' }}>
                    Add to Cart
                  </button>
                </div>
              ))
        ...
        <main style={{ maxWidth: '500px', margin: '20px auto', padding: '0 15px' }}>
          {!isVendorRegistered ? (
            <form onSubmit={handleVendorRegister} style={{ backgroundColor: '#0f172a', padding: '25px', borderRadius: '16px', border: '1px solid #1e293b' }}>
              <h3 style={{ margin: '0 0 8px 0', color: '#34d399', fontSize: '18px' }}>🏪 Vendor Store Registration</h3>
              <p style={{ fontSize: '12px', color: '#94a3b8', marginBottom: '20px' }}>Register your store details to start publishing products.</p>
            ...
            <form onSubmit={addProd} style={{ backgroundColor: '#0f172a', padding: '25px', borderRadius: '16px', border: '1px solid #1e293b' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                <div>
                  ...
                  <span style={{ fontSize: '11px', color: '#38bdf8', fontWeight: 'bold' }}>Store: {storeName}</span>
                </div>
            ...
            {cart.length === 0 ? (
              ...
              <div>
                <div style={{ maxHeight: '200px', overflowY: 'auto', marginBottom: '15px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  {cart.map((item, idx) => (
                    <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#020617', padding: '10px 12px', borderRadius: '8px', border: '1px solid #1e293b' }}>
                      <div>
                        <span style={{ fontSize: '12px', fontWeight: 'bold', color: '#fff', display: 'block' }}>{item.name}</span>
                        <span style={{ fontSize: '10px', color: '#94a3b8' }}>Store: {item.store}</span>
                      </div>
                      <span style={{ color: '#38bdf8', fontWeight: 'bold', fontSize: '12px' }}>{sym[cur]} {conv(item.price)}</span>
                    </div>
                  ))}
                </div>
                <div style={{ borderTop: '1px solid #1e293b', paddingTop: '12px', display: 'flex', justifyContent: 'space-between', marginBottom: '20px', fontWeight: 'bold' }}>
                  <span style={{ fontSize: '13px', color: '#cbd5e1' }}>Total:</span>
      ...
      {selectedProduct && (
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.8)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 50, padding: '15px' }}>
          <div style={{ backgroundColor: '#0f172a', border: '1px solid #1e293b', width: '100%', maxWidth: '400px', borderRadius: '16px', padding: '20px' }}>
            ...
            <button onClick={() => { setCart([...cart, selectedProduct]); setSelectedProduct(null); }} style={{ width: '100%', backgroundColor: '#2563eb', color: '#fff', border: 'none', padding: '12px', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer', fontSize: '14px' }}>
              Add to Cart
            </button>
          </div>
        </div>
